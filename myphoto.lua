...
-- "blend"    , "set blending factor (0 always duplicates frames)", OFFSET(blend_factor), AV_OPT_TYPE_FLOAT, {.dbl=1}, 0, FLT_MAX, FLAGS },
--]]
...
-- blending factor (1 = default, 0 = off)
b = {}
	b[1] = "0"
	b[2] = "0.5"
	b[3] = "1"
...
function apply(ftype, adjust)
	...
	elseif ftype == "bypass" then
		...
		elseif ftype == "b" then blend	= (b[val["b"]]) ; updateFilter("photo")
		elseif ftype == "r" then
			height = (r[val["r"]])
...
function updateFilter(filter, duration)
	...
	toggleMsg("vf=no", 1)
	local photo = "@photo:photosensitivity=t=" .. thresh .. ":f=" .. frames .. ":skip=" .. skip .. ":blend=" .. blend
	-- '[aid1]anull[ao];' was breaking videos with no audio, removed
	if graph_compare == 0 then
		graph = "[vid1]scale=w=960:h=-2,photosensitivity=t=" .. thresh .. ":f=" .. frames .. ":skip=" .. skip .. ":blend=" .. blend .. ",split[P][I]; [I]drawgraph=fg1=0x00FFFF:m1=lavfi.photosensitivity.fixed-badness:fg2=0x0000FF:m2=lavfi.photosensitivity.badness:fg3=0xFF00FF:m3=lavfi.photosensitivity.frame-badness:fg4=0x00FF00:m4=lavfi.photosensitivity.factor:min=0:max=2:slide=scroll:bg=0x000000:mode=line:size=960x100[G]; [P][G]vstack[vo]"
	else
		graph = "[vid1]scale=w=640:h=-2, split[A][B]; [B]photosensitivity=t=" .. thresh .. ":f=" .. frames .. ":skip=" .. skip .. ":blend=" .. blend .. "[P]; [P]split[R][I]; [A][R]hstack[C]; [I]drawgraph=fg1=0x00FFFF:m1=lavfi.photosensitivity.fixed-badness:fg2=0x0000FF:m2=lavfi.photosensitivity.badness:fg3=0xFF00FF:m3=lavfi.photosensitivity.frame-badness:fg4=0x00FF00:m4=lavfi.photosensitivity.factor:min=0:max=2:slide=scroll:bg=0x000000:mode=line:size=1280x100[G]; [C][G]vstack[vo]"
	end
...
mp.add_forced_key_binding("shift+F6", "fDecr", function()	apply("f", -1); end)
mp.add_forced_key_binding("F9", "bHigher", function()	apply("b", 1); end)
mp.add_forced_key_binding("shift+F9", "bDecr", function()	apply("b", -1); end)
